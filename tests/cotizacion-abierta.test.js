...
test("la cantidad elegida manda el precio y el anticipo es el 50%", async () => {
  ...
  // 3 x US$25,500 de lista = US$76,500, con la mitad de anticipo.
  assert.match(mail.text, /Total US\$76,500\.00/);
  assert.match(mail.text, /anticipo US\$38,250\.00/);
  assert.match(mail.text, /saldo US\$38,250\.00/);
});
...
test("el precio no se toma del navegador por más que lo mande", async () => {
  ...
  assert.match(mail.text, /Total US\$25,500\.00/);
});
...
test("una cantidad fuera de rango se recorta al máximo permitido", async () => {
  ...
  assert.match(mail.text, /Total US\$127,500\.00/);
});
...
test("el cliente elige el modelo y manda el precio de ese modelo", async () => {
  ...
  // 4-Stall = US$31,500 de lista.
  assert.match(mail.text, /Total US\$31,500\.00/);
...
test("un modelo inventado cae en el que trae la cotización", async () => {
  ...
  assert.equal(status, 200);
  assert.match(mail.text, /Total US\$25,500\.00/); // el 3-Stall por defecto
});
...
test("el ADA+2 se firma como cualquier otro", async () => {
  ...
  assert.equal(status, 200);
  assert.match(mail.text, /Total US\$30,500\.00/);
  assert.equal(mail.attachments.length, 1);
});
